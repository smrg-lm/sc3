...
class Convolution(ugn.UGen):
    ...
    @classmethod
    def ar(cls, input, kernel, frame_size=512, mul=1.0, add=0.0):
        return cls._multi_new('audio', input, kernel, frame_size).madd(mul, add)
...
class Convolution2(ugn.UGen):
    ...
    @classmethod
    def ar(cls, input, kernel, trigger=0, frame_size=2048, mul=1.0, add=0.0):  # NOTE: wrong kernel size crashes the server.
        return cls._multi_new('audio', input, kernel, trigger,
                              frame_size).madd(mul, add)
...
class Convolution2L(ugn.UGen):
    ...
    def ar(cls, input, kernel, trigger=0, frame_size=2048, crossfade=1,
        ...
        return cls._multi_new('audio', input, kernel, trigger, frame_size,
                              int(crossfade)).madd(mul, add)


class StereoConvolution2L(ugn.MultiOutUGen):
    # // Fixed kernel stereo convolver with linear crossfade.
    @classmethod
    def ar(cls, input, kernel_L, kernel_R, trigger=0, frame_size=2048,
           crossfade=1, mul=1.0, add=0.0):
        return cls._multi_new('audio', input, kernel_L, kernel_R, trigger,
                              frame_size, int(crossfade)).madd(mul, add)
...
class Convolution3(ugn.UGen):
    ...
    @classmethod
    def ar(cls, input, kernel, trigger=0, frame_size=2048, mul=1.0, add=0.0):
        return cls._multi_new('audio', input, kernel, trigger,
                              frame_size).madd(mul, add)

    @classmethod
    def kr(cls, input, kernel, trigger=0, frame_size=2048, mul=1.0, add=0.0):
        return cls._multi_new('control', input, kernel, trigger,
                              frame_size).madd(mul, add)
