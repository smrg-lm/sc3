...
# así podría andar y simplemente hay que agregar el register en cada clase
# y no acá.
...
class UGen(AbstractFunction):
    ...
    def new1(cls, rate, *args): # la verdad que see podría llamar single_new.
        ...
        obj.rate = rate
        obj.add_to_synth()
        return obj.init(*args) # debería llamarse init_ugen o algo así
    ...
    def __init__(self):
        ...
        self.rate = 'audio' # *** !!! hacer un enum de algún tipo !!!
        ...
        # topo sorting
        self.antecedents = set() # estos sets los inicializa SynthDef init_topo_sort
        ...
        if self.rate is 'audio': # *** convertir audio en constante de enum
            ...
            for i in range(n):
                if as_ugen_rate(self.inputs[i]) != 'audio': # *** VER VALORES POSIBLES PARA self.inputs[i]:: IMPLEMENTADO ARRIBA COMO SINGLE DISPATCH
                    return 'input {} is not audio rate: {} {}'
    ...
    def check_sr_as_first_input(self): # checkSameRateAsFirstInput ídem anterior, deben ser interfaz protejida
        if self.rate is not as_ugen_rate(self.inputs[0]): # *** VER VALORES POSIBLES PARA self.inputs[0]: IMPLEMENTADO ARRIBA COMO SINGLE DISPATCH
            return 'first input is not {} rate: {} {}'
    ...
    def arg_name_for_input_at(self, i): # se usa acá y en basicopugen dentro de checkValidInputs, ambas clases lo implementan.
        try:
            ...
            if i < len(arg_names):
                if selector is '__init__':
                    return arg_names[i + 1] # TODO: VER: 1 es arg_names_inputs_offset
                else:
                    return arg_names[i]
            else:
    ...
    # TODO: Si se usa __init__ como new de sclang *sí* se necesita offset. Los métodos mágicos devuelven self/cls.
    ...
    def method_selector_for_rate(cls, rate): # este tiene una variante de instancia, ahora acá arriba.
        ...
        if rate is 'scalar':
            if 'ir' in dir(cls): # VER arriba: es try: getattr(cls, self.method_selector_for_rate()) except AttributeError: lala.
                ...
                return '__init__'
        if rate is 'demand': return '__init__'
        return None
    ...
    #*replaceZeroesWithSilence # TODO es recursiva y la usan Function-asBuffer, (AtkMatrixMix*ar), GraphBuilder-wrapOut, LocalOut*ar, Out*ar, XOut*ar.
    ...
    def init_topo_sort(self):
        for input in self.inputs:
            if isinstance(input, UGen):
                self.antecedents.add(input.source) # TODO: source es un método/propiedad de varias clases, Array (que returns the source UGen from an Array of OutputProxy(s)) y Nil
                input.source.descendants.add(self) # TODO: source, Object (devuelve this), Nil (método vacío), OutputProxy (es propiedad) y Array, VER otras clases
                                                   # TODO: debería llamarse source_ugen o get_source_ugen, source es muy común para algo tan específico. Y es interfaz.
    ...
    def optimize_graph(self):
        return # pass? se usa para esto o es confuso?
...
# NO SÉ SI REFIERE A PUREUGEN,  debe ser... No sé si es necesario reimplemntar perform_dead_code_elimination para llamar a super.perform_dead_code_elimination...
