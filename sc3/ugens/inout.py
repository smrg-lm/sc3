...
class Control(AbstractControl):
    ...
    def _init_ugen(self, *values):  # override
        ...
        if self._synthdef is not None:
            self._special_index = len(self._synthdef.controls)  # TODO: VER, esto se relaciona con _Symbol_SpecialIndex como?
            self._synthdef.controls.extend(self.values)

            ctl_names = self._synthdef.control_names
            if len(ctl_names) > 0:
            ...
            self._synthdef.control_index += len(self.values)
...
class AudioControl(AbstractControl):
    ...
    @classmethod
    def names(cls, names):
        synthdef = _gl.current_synthdef  # VER: lo mimso que arriba, es local, luego _init_ugen llama a self._synthdef, supongo que se inicializa en las subclases.
        index = synthdef.control_index
        names = utl.as_list(names)
        for i, name in enumerate(names):
            synthdef.add_control_name(
